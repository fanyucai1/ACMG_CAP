...
def run(pe1,pe2,outdir,prefix,bed):
    ...
    subprocess.check_call(cmd, shell=True)
    cmd = "cd %s && %s -Xmx20G -jar %s --in %s.dup.bam --out %s.abra.bam --ref %s --kmer 43 --mad 5000 --targets %s --threads 10 --tmpdir ./ >abra.log && rm %s.dup.bam %s.dup.bam.bai && %s index %s.abra.bam" \
          % (outdir, java, abra, out, out, ref, bed,out,out,samtools,out)
    subprocess.check_call(cmd,shell=True)
